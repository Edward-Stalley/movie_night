...
export default function ChosenByInput({ movie, users }: { movie: WatchedMovie; users: User[] }) {
  ...
  const handleDropdown = async (user: User) => {
    console.log('name clicked', user);
  ...
  return (
    <div className="flex pt-1 pb-1  items-center   ">
      <div className="flex justify-center items-center rounded-2xl bg-base-300 h-8 gap-2 ">
        <div className="flex justify-center pitems-center w-fit pl-2  ">
          <div>
            <div className=" h-8 flex justify-center items-center">
              <button
                className="btn m-1 h-6 "
              ...
              >
                {users.map((user) => (
                  <li key={`${user.id}-${user.name}`}>
                    <a onClick={() => handleDropdown(user)}>{user.name}</a>
                  </li>
                ))}
              </ul>
            </div>
